...
public class WurstplusFrame {
	...
	public WurstplusFrame(WurstplusCategory category) {
		...
		int size  = FloppaHack.get_hack_manager().get_modules_with_category(category).size();
		int count = 5;

		for (WurstplusHack modules : FloppaHack.get_hack_manager().get_modules_with_category(category)) {
	...
	public void refresh_frame(WurstplusModuleButton button, int combo_height) {
		...
		int size  = FloppaHack.get_hack_manager().get_modules_with_category(this.category).size();
		int count = 0;
	...
	public void crush(int mx, int my) {
		...
		int screen_x = (mc.displayWidth + 1);
		int screen_y = (mc.displayHeight + 1);
	...
	public void render(int mx, int my) {
		...
		int nc_r = FloppaHack.click_gui.theme_frame_name_r;
		int nc_g = FloppaHack.click_gui.theme_frame_name_g;
		int nc_b = FloppaHack.click_gui.theme_frame_name_b;
		int nc_a = FloppaHack.click_gui.theme_frame_name_a;

		int bg_r = FloppaHack.click_gui.theme_frame_background_r;
		int bg_g = FloppaHack.click_gui.theme_frame_background_g;
		int bg_b = FloppaHack.click_gui.theme_frame_background_b;
		int bg_a = FloppaHack.click_gui.theme_frame_background_a;

		int bd_r = FloppaHack.click_gui.theme_frame_border_r;
		int bd_g = FloppaHack.click_gui.theme_frame_border_g;
		int bd_b = FloppaHack.click_gui.theme_frame_border_b;
		int bd_a = border_a;
