...
public class WurstplusOffhand extends WurstplusHack {
    ...
    WurstplusSetting switch_mode = create("Offhand", "OffhandOffhand", "Gapple", combobox("Totem", "Crystal", "Gapple"));
    WurstplusSetting totem_switch = create("Totem HP", "OffhandTotemHP", 10, 0, 36);

    WurstplusSetting gapple_in_hole = create("Gapple In Hole", "OffhandGapple", false);
    WurstplusSetting gapple_hole_hp = create("Gapple Hole HP", "OffhandGappleHP", 5, 0, 36);
    ...
    public void update() {
        ...
        if (mc.currentScreen == null || mc.currentScreen instanceof GuiInventory) {
            ...
            if (hp > totem_switch.get_value(1)) {
                if (switch_mode.in("Crystal") && FloppaHack.get_hack_manager().get_module_with_tag("AutoCrystal").is_active()) {
                ...
                if (switch_mode.in("Crystal") && !FloppaHack.get_hack_manager().get_module_with_tag("AutoCrystal").is_active()) {
                    swap_items(get_item_slot(Items.END_CRYSTAL),1);
                    return;
                }
            } else {
                swap_items(get_item_slot(Items.TOTEM_OF_UNDYING), delay.get_value(true) ? 1 : 0);
